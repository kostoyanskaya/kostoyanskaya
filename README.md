...
| [QRKot](https://github.com/kostoyanskaya/qr_code) | A REST API for a charity fund that collects donations and distributes them across open charity projects. When a new project is created, available donations are automatically invested into it; when a new donation is made, it is automatically distributed among open projects. The API supports user registration and JWT authentication, with separate access levels for regular users and superusers (only superusers can create, update, or delete charity projects and view the full list of donations). Database migrations are managed with Alembic, and the API comes with automatically generated interactive documentation. | FastAPI, SQLAlchemy (async), Pydantic, fastapi-users, Alembic, SQLite |
| [[DocMind (in progress)](https://github.com/kostoyanskaya/search_documents) | DocMind is a full-featured RAG (Retrieval-Augmented Generation) application that helps users find the right document based on their queries or input data. The agent retrieves the most relevant excerpts, passes them to an LLM as grounded context, and returns a response along with precise citations from the sources it relies on—rather than letting the model guess or fabricate information. Users can upload up to 50 documents at once (.pdf, .docx, .csv, .xlsx, .xls). The system automatically finds the relevant document, displays its title, pinpoints exactly where the requested information is located, and provides a clear answer with source attribution. Each user only ever sees their own documents — data is fully isolated, and nothing is ever written to disk. The backend is built with FastAPI, LangChain + LangGraph for RAG orchestration, ChromaDB as an ephemeral vector store, Sentence-Transformers for local multilingual embeddings, and DeepSeek / OpenAI-compatible API for answer generation. The frontend is built with React 18, TypeScript, and Vite, with plain CSS and cookie-based session auth. | FastAPI, LangChain, LangGraph, ChromaDB, Sentence-Transformers, DeepSeek, OpenAI API, pypdf, python-docx, pandas, openpyxl, Pydantic, React, TypeScript, Vite |
